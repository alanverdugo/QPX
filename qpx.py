...
def read_config():
    ...
    try:
        ...
        log.error("Unable to read configuration file. %s Exception"\
                  ": %s", config_file, exception)
        sys.exit(1)
...
def send_email(results_message, origin_city, destination_city, sale_total):
    ...
    try:
        ...
        log.info("Success! Notification email sent to: %s", email_to)
        log.info("Message: %s", results_message)
        sys.exit(0)
...
def main(origin1, destination1, departure_date, duration, delay, solutions,
    ...
    if duration:
        ...
        payload = '''{{"request":{{"passengers":{{"adultCount":{0}}},
            ...
            "maxPrice":"{7}","solutions":{8}}}}}'''.format(adults,
                                                           origin1,
                                                           destination1,
                                                           str(date1),
                                                           origin2,
        ...
        payload = '''{{"request":{{"passengers":{{"adultCount":{0}}},
            "slice":[{{"origin":"{1}","destination":"{2}","date":"{3}"}}],
            "maxPrice":"{4}","solutions":{5}}}}}'''.format(adults,
                                                           origin1,
                                                           destination1,
                                                           str(date1),
                                                           max_price,
    ...
    try:
        ...
        log.error("Unable to execute request: %s", exception)
        sys.exit(1)

    # The status code should be 200 (success). Catch anything else and handle.
    if response.status_code != 200:
        log.error("The response status code is: %s Reason: "\
    ...
    try:
        ...
        log.error("Empty result set. Payload: %s", payload)
        sys.exit(2)
